...
class TextToVoice:
    ...
    def from_payload(cls, payload: object) -> "TextToVoice | None":
        if not isinstance(payload, dict):
            return None
        return cls(*(value if isinstance(value, str) else None for value in (
            payload.get("beforePasswordText"), payload.get("password"), payload.get("language"),
        )))
...
class DeliveryContext:
    ...
    def from_payload(cls, payload: object) -> "DeliveryContext | None":
        ...
        return cls(
            ...
            text_to_voice=TextToVoice.from_payload(voice.get("text2voice")) if isinstance(voice, dict) else None,
        )
